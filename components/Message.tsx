...
export default function Message({ message }: { message: UIMessage }) {
  ...
  return (
    <div className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        {!toolResults.some((t: any) => t.toolName === 'get_current_weather') && textContent && (
          <p
            className={`whitespace-pre-wrap text-[15px] leading-relaxed ${isUser ? 'text-white' : 'text-gray-100'
              }`}
          >
            {textContent}
          </p>
